...
func main() {
	...
	cleanup := true
	insecure := true // Set insecure to true for local testing
	...
	// Use NewClientForConfig to pass the config directly instead of loading from file
	// This avoids issues with pointer fields like Cleanup not being properly loaded
	fmt.Printf("Rancher config: %+v\n", rancherConfig)
	fmt.Printf("Test session: %+v\n", testSession)
	fmt.Printf("Cluster name: %s\n", clusterName)
	fmt.Printf("File path: %s\n", os.Getenv("CATTLE_TEST_CONFIG"))

	client, err = rancherClient.NewClientForConfig("", rancherConfig, testSession)
	if err != nil {
