...
import (
	...
	kubenamespaces "github.com/rancher/steve/tests/integration/actions/kubeapi/namespaces"
	"github.com/rancher/steve/tests/integration/actions/kubeapi/rbac"
	"github.com/rancher/steve/tests/integration/actions/kubeapi/secrets"
	stevesecrets "github.com/rancher/steve/tests/integration/actions/secrets"
	"github.com/rancher/steve/tests/integration/actions/serviceaccounts"

	"github.com/rancher/steve/tests/integration/actions/namespaces"
...
func (s *steveAPITestSuite) setupSuite(clusterName string) {
	...
	// Load config first to ensure pointer fields are set
	rancherConfig := new(rancher.Config)
	config.LoadConfig(rancher.ConfigurationFileKey, rancherConfig)

	// Debug: log config values
	s.T().Logf("Loaded config - Host: %s, AdminToken: %s (len: %d)",
		rancherConfig.Host,
		func() string {
			if rancherConfig.AdminToken == "" {
				return "<empty>"
			}
			return rancherConfig.AdminToken[:20] + "..."
		}(),
		len(rancherConfig.AdminToken))

	// Ensure Host is set (it might be empty when loaded from YAML)
	if rancherConfig.Host == "" {
		// Try to get host from environment or use default
		if host := os.Getenv("CATTLE_SERVER_URL"); host != "" {
			// Remove https:// prefix if present
			host = strings.TrimPrefix(host, "https://")
			host = strings.TrimPrefix(host, "http://")
			rancherConfig.Host = host
		} else {
			s.T().Fatalf("CATTLE_SERVER_URL is not set")
		}
	}

	// Ensure Cleanup is set (it might be nil when loaded from YAML)
	if rancherConfig.Cleanup == nil {
		cleanup := true
		rancherConfig.Cleanup = &cleanup
	}

	// Ensure Insecure is set (it might be nil when loaded from YAML)
	if rancherConfig.Insecure == nil {
		insecure := true // Default to insecure for local testing
		rancherConfig.Insecure = &insecure
	}

	// Ensure AdminToken is set (use bearerToken parameter if AdminToken is empty)
	bearerToken := ""
	if rancherConfig.AdminToken != "" {
		bearerToken = rancherConfig.AdminToken
	}

	// Use NewClientForConfig to pass the config directly
	...
	fmt.Printf("Project map: %+v\n", projectMap)
	fmt.Printf("Project namespace map: %+v\n", projectNamespaceMap)
	fmt.Printf("Namespace map: %+v\n", namespaceMap)

	client, err := rancher.NewClientForConfig(bearerToken, rancherConfig, testSession)
	require.NoError(s.T(), err)
	s.client = client
	...
	for n := range namespaceMap {
		...
		err = scheme.Convert(resp, ns, resp.GroupVersionKind())
		require.NoError(s.T(), err)
...
func setUpResults() (*csv.Writer, *os.File, string, error) {
	...
	csvFile, err := os.OpenFile(outputFile, os.O_RDWR|os.O_CREATE|os.O_TRUNC, 0o644)
	if err != nil {
		return nil, nil, "", err
	}
...
func writeResp(csvWriter *csv.Writer, user, url, path string, resp []byte) error {
	jsonFile, err := os.OpenFile(path, os.O_RDWR|os.O_CREATE|os.O_TRUNC, 0o644)
	if err != nil {
		return err
	}
	jsonFile.Write(resp)
