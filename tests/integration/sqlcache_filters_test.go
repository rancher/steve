...
func (i *IntegrationSuite) TestSQLCacheFilters() {
	...
	testFile := filepath.Join("testdata", "sqlcache", "filters.test.yaml")

	i.testFiltersScenario(ctx, testFile, baseURL)
}

func (i *IntegrationSuite) testFiltersScenario(ctx context.Context, testFile string, baseURL string) {
