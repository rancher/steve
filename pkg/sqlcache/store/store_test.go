...
func TestAddWithOneUpdate(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.description, func(t *testing.T) {
			...
			c.EXPECT().Prepare(WSIgnoringMatcher(rawStmt2)).Return(preparedStmt, nil)
			...
			c.EXPECT().Prepare(rawStmt2a).Return(preparedStmt, nil)
			...
			c.EXPECT().Prepare(rawStmt4).Return(preparedStmt, nil)
			txC.EXPECT().Stmt(preparedStmt).Return(stmts)
...
func TestAddWithExternalUpdates(t *testing.T) {
	...
	tests = append(tests, testCase{description: "Add with no DB client errors", test: func(t *testing.T) {
		...
		c.EXPECT().Prepare(WSIgnoringMatcher(rawStmt1b)).Return(preparedStmt, nil)
		...
		c.EXPECT().Prepare(rawStmt2).Return(preparedStmt, nil)
		txC.EXPECT().Stmt(preparedStmt).Return(stmts)
		preparedStmt.EXPECT().Close()
		stmts.EXPECT().Exec("moose1", "lego.cattle.io/fields1")
		...
		c.EXPECT().Prepare(WSIgnoringMatcher(rawStmt3b)).Return(preparedStmt, nil)
