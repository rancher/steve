...
func TestNewIndexer(t *testing.T) {
	...
	tests = append(tests, testCase{description: "NewIndexer() with no errors returned from Store or Client, should return no error", test: func(t *testing.T) {
		...
		store.EXPECT().Prepare(fmt.Sprintf(deleteIndicesFmt, storeName)).Return(stmt, nil)
		store.EXPECT().Prepare(fmt.Sprintf(dropIndicesFmt, storeName)).Return(stmt, nil)
		store.EXPECT().Prepare(fmt.Sprintf(listByIndexFmt, storeName, storeName)).Return(stmt, nil)
		store.EXPECT().Prepare(fmt.Sprintf(listKeyByIndexFmt, storeName)).Return(stmt, nil)
		store.EXPECT().Prepare(fmt.Sprintf(listIndexValuesFmt, storeName)).Return(stmt, nil)
...
func TestIndex(t *testing.T) {
	...
	tests = append(tests, testCase{description: "Index() with no errors returned from store and multiple keys returned from index func, should return one obj and no error", test: func(t *testing.T) {
		...
		store.EXPECT().Prepare(fmt.Sprintf(selectQueryFmt, "name", ", ?")).Return(mockStmt, nil)
