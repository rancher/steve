...
func TestSimpleFoodFieldArrayQueries(t *testing.T) {
	...
	tests = append(tests, testCase{
		...
		query:           "filter=recipe contains taco",
		expectedResults: []string{"hamburgers", "hot dogs"},
	})
	tests = append(tests, testCase{
		description:     "find sandwiches, sorting descending",
		query:           "filter=recipe contains sandwiches&sort=-metadata.name",
		expectedResults: []string{"hot dogs", "hamburgers", "duck", "croissants"},
	})
	tests = append(tests, testCase{
		description:     "find cake or pie",
		query:           "filter=recipe contains cake,recipe contains pie",
		expectedResults: []string{"lemons", "limes", "mangoes", "plums"},
	})
	tests = append(tests, testCase{
		description:     "or-bar-searching",
		query:           "filter=brands contains kroger",
		expectedResults: []string{"bacon", "duck", "hot dogs", "tacos"},
	})
	...
	for _, test := range tests {
		t.Run(test.description, func(t *testing.T) {
			...
			fields := []string{"metadata.name", "foodCode", "country", "state", "recipe[1]", "recipe[2]", "recipe[3]", "recipe[4]", "brands"}
			l := &ListOptionIndexer{
				Indexer:       i,
				namespaced:    false,
				indexedFields: fields,
