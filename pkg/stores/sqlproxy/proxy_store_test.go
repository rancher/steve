...
func TestNewProxyStore(t *testing.T) {
	...
	tests = append(tests, testCase{
		...
		test: func(t *testing.T) {
			...
			cf.EXPECT().CacheFor([][]string{{`id`}, {`metadata`, `state`, `name`}}, gomock.Any(), &tablelistconvert.Client{ResourceInterface: ri}, attributes.GVK(&nsSchema), false, false).Return(c, nil)
	...
	tests = append(tests, testCase{
		...
		test: func(t *testing.T) {
			...
			scc.EXPECT().SetColumns(context.Background(), &nsSchema).Return(nil)
			cg.EXPECT().TableAdminClient(nil, &nsSchema, "", &WarningBuffer{}).Return(ri, nil)
			cf.EXPECT().CacheFor([][]string{{`id`}, {`metadata`, `state`, `name`}}, gomock.Any(), &tablelistconvert.Client{ResourceInterface: ri}, attributes.GVK(&nsSchema), false, false).Return(factory.Cache{}, fmt.Errorf("error"))
...
func TestListByPartitions(t *testing.T) {
	...
	tests = append(tests, testCase{
		...
		test: func(t *testing.T) {
			...
			// This tests that fields are being extracted from schema columns and the type specific fields map
			tb.EXPECT().GetTransformFunc(attributes.GVK(schema)).Return(func(obj interface{}) (interface{}, error) { return obj, nil })
			cf.EXPECT().CacheFor([][]string{{"some", "field"}, {`id`}, {`metadata`, `state`, `name`}, {"gvk", "specific", "fields"}}, gomock.Any(), &tablelistconvert.Client{ResourceInterface: ri}, attributes.GVK(schema), attributes.Namespaced(schema), false).Return(factory.Cache{}, fmt.Errorf("error"))
...
func TestReset(t *testing.T) {
	...
	tests = append(tests, testCase{
		...
		test: func(t *testing.T) {
			...
			cg.EXPECT().TableAdminClient(nil, &nsSchema, "", &WarningBuffer{}).Return(ri, nil)
			cf.EXPECT().CacheFor([][]string{{`id`}, {`metadata`, `state`, `name`}}, gomock.Any(), &tablelistconvert.Client{ResourceInterface: ri}, attributes.GVK(&nsSchema), false, false).Return(nsc2, nil)
