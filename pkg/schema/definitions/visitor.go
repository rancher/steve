...
// VisitKind turns a kind into a definitionField (stored on the receiver) and a definition. Both are stored on the receiver.
func (s *schemaFieldVisitor) VisitKind(kind *proto.Kind) {
	...
	// the visitor may have set the field mutliple times while evaluating kind fields, so we only set the final
