...
func (s *Store) Watch(apiOp *types.APIRequest, schema *types.APISchema, w types.WatchRequest) (chan types.APIEvent, error) {
	...
	counts = s.getCount(apiOp, schema).Counts
	for id := range counts {
...
func getInfo(obj interface{}, schema *types.APISchema) (name string, namespace string, revision int, summaryResult summary.Summary, ok bool) {
	...
	summaryResult = summary.Summarize(r, opts)
...
func (s *Store) getCount(apiOp *types.APIRequest, schema *types.APISchema) Count {
	counts := map[string]ItemCount{}
