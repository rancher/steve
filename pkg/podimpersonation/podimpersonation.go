...
func (s *PodImpersonation) augmentPod(pod *v1.Pod, sa *v1.ServiceAccount, secret *v1.Secret, imageOverride string) *v1.Pod {
	...
	pod.Spec.Containers = append(pod.Spec.Containers, v1.Container{
		...
		Env: []v1.EnvVar{
			{
				Name:  "KUBECONFIG",
				Value: userKubeConfigPath("shell"), // TODO: maybe we need a way to configure user?
			},
		},
		...
		VolumeMounts: []v1.VolumeMount{
			{
				Name:      "admin-kubeconfig",
				ReadOnly:  true,
				MountPath: userKubeConfigPath("shell"), // TODO: maybe we need a way to configure user?
				SubPath:   "config",
			},
			{
				Name:      secret.Name,
...
func userKubeConfigPath(user string) string {
	if user == "root" {
